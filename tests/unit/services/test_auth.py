...
class TestRoleCheck:
    ...
    async def test_allowed_role_passes(self, auth_service_, session):
        ...
        with (
            patch.object(auth_service, "get_token_from_request", return_value="tok"),
            patch.object(
                auth_service, "get_current_user",
                new=AsyncMock(return_value=mock_user),
            ),
        ):
            result = await checker(session=session, request=MagicMock())

        assert result is mock_user

    async def test_wrong_role_raises_403(self, auth_service_, session):
        auth_service = auth_service_
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc:
                await checker(session=session, request=MagicMock())

        assert exc.value.status_code == 403

    async def test_empty_allowed_roles_denies_all(self, auth_service_, session):
        auth_service = auth_service_
...
class TestGetTokenFromRequest:
    """AuthService.get_token_from_request — реальная логика парсинга заголовков."""
    ...
    def test_no_auth_header_falls_to_cookie(self, auth_service_):
        ...
        with patch.object(
            auth_service, "_get_token_from_cookie", return_value="cookie_token",
        ) as mock_cookie:
            result = auth_service.get_token_from_request(request)
    ...
    def test_cookie_raises_when_no_token(self, auth_service_):
        ...
        with patch.object(
            auth_service, "_get_token_from_cookie",
...
class TestLogin:
    ...
    async def test_user_not_found_skips_password_check(self, auth_service_, session):
        """Short-circuit OR: если user None, verify_password НЕ вызывается."""
        auth_service = auth_service_

        with (
            patch.object(
                auth_service, "get_by_username",
            ...
            with pytest.raises(HTTPException) as exc:
                await auth_service.login(
                    session, "nobody", "anypass", MagicMock(),
                )
    ...
    async def test_delete_user_password_raises_attribute_error(
        ...
        with (
            ...
            with pytest.raises(AttributeError):
                await auth_service.login(
                    session, "hacker", "anypass", MagicMock(),
                )
    ...
    async def test_inactive_user_raises_403_before_token_creation(
        ...
        with (
            ...
            patch.object(
                auth_service.token_service, "create_tokens", mock_create_tokens,
            ),
        ):
            with pytest.raises(HTTPException) as exc:
                await auth_service.login(
                    session, "blocked", "pass", MagicMock(),
                )
