...
class TestCheckPermissions:
    ...
    def test_admin_can_access_any_appointment(self, appointment_service_, admin_user):
    ...
    def test_owner_can_access_own_appointment(self, appointment_service_, regular_user):
        service = appointment_service_
        appointment = MagicMock()
    ...
    def test_master_role_obeys_same_rule(self, appointment_service_, master_user):
        service = appointment_service_
...
class TestCheckTimeAvailable:
    ...
    async def test_past_time_raises_400(self, appointment_service_, session):
        service = appointment_service_
    ...
    async def test_same_day_passes_past_check(self, appointment_service_, session):
        ...
        await service.check_time_available(
            appointment_time=today, session=session, duration=timedelta(hours=1),
        )

    async def test_overlap_raises_409(self, appointment_service_, session):
        service = appointment_service_
        mock_result = MagicMock()
        ...
        with pytest.raises(HTTPException) as exc:
            await service.check_time_available(
                appointment_time=FUTURE, session=session,
                duration=timedelta(hours=1),
            )

        assert exc.value.status_code == 409

    async def test_adjacent_times_are_ok(self, appointment_service_, session):
    ...
    async def test_exclude_id_ignores_conflict(self, appointment_service_, session):
        service = appointment_service_
        mock_result = MagicMock()
        mock_result.scalars.return_value.first.return_value = None
        session.execute.return_value = mock_result
        ...
        await service.check_time_available(
            appointment_time=FUTURE, session=session,
            duration=timedelta(hours=1), exclude_id=999,
        )

    async def test_non_scheduled_overlap_is_ok(
        self, appointment_service_, session,
    ):
...
class TestUpdateAppointment:
    """AppointmentService.update_appointment — ветвление по appointment_time."""

    async def test_skips_time_check_when_no_time_provided(
        self, appointment_service_, session, regular_user,
    ):
        service = appointment_service_
        request = MagicMock()
    ...
    async def test_checks_time_when_time_provided(
        ...
        with (
            patch.object(service, "get_object_or_404", AsyncMock(return_value=mock_appointment)),
            patch.object(
                service.master_service_manager, "get_object_or_404",
        ...
        mock_check_time.assert_called_once_with(
            FUTURE, session, master_service.duration,
        )
    ...
    async def test_raises_403_for_others(
        ...
        with patch.object(
            ...
            with pytest.raises(HTTPException) as exc:
                await service.update_appointment(
                    request=MagicMock(), session=session,
                    appointment_id=1, user=regular_user,
                )
...
class TestAdminCreateAppointment:
    async def test_creates_via_create_appointment(self, appointment_service_, session):
        service = appointment_service_
        request = MagicMock()
        request.model_dump.return_value = {
            "appointment_time": FUTURE, "master_service_id": 1, "user_id": 1,
        }
        ...
        with (
            patch.object(
                service, "_validate_and_get_service_and_master",
                new=AsyncMock(),
            ),
            patch.object(service.crud, "create", new=AsyncMock(return_value=mock_appointment)),
            patch("app.services.appointment.send_appointment_notification.delay"),
        ):
            result = await service.admin_create_appointment(request=request, session=session)
            assert result is mock_appointment


class TestAdminUpdateAppointment:
    async def test_updates_time_and_master_service(self, appointment_service_, session):
        service = appointment_service_
        request = MagicMock()
        request.appointment_time = FUTURE
        ...
        with (
            patch.object(service, "get_object_or_404", new=AsyncMock(return_value=mock_appointment)),
            patch.object(
                service.master_service_manager, "get_object_or_404",
                new=AsyncMock(return_value=new_master_service),
            ),
            patch.object(service, "check_time_available") as mock_check_time,
            patch.object(service.crud, "update", new=AsyncMock(return_value=mock_appointment)),
        ):
            result = await service.admin_update_appointment(
                request=request, session=session, appointment_id=1,
            )
            assert result is mock_appointment
            mock_check_time.assert_called_once()

    async def test_skips_time_check_when_no_changes(self, appointment_service_, session):
        service = appointment_service_
        request = MagicMock()
        request.appointment_time = None
...
class TestMasterUpdateAppointment:
    async def test_updates_status(self, appointment_service_, session):
        service = appointment_service_
        request = MagicMock()
...
class TestValidateAndGetServiceAndMaster:
    async def test_returns_master_service(self, appointment_service_, session):
        service = appointment_service_
