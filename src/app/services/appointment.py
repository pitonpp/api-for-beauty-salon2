...
class AppointmentService(BaseService[Appointment, CRUDAppointment]):
    ...
    def _check_permissions(
        ...
        """Проверяет, что пользователь имеет права на запись (своя или админ)."""

        if user.role != UserRole.ADMIN and appointment.client_id != user.id:
            logger.warning(
                "Пользователь user_id={} role={} не имеет прав на редактирование записи appointment_id={}",
                user.id,
                user.role,
                appointment.id,
            )
    ...
    async def _create_appointment(
        ...
        """Внутренний метод создания записи с проверкой времени и отправкой уведомления."""
    ...
    async def admin_update_appointment(
        ...
        """Обновляет запись администратором (может менять время, услугу, статус)."""
